...
export function Toolbar({ store, openDialog, canvasApi }: Props) {
  ...
  return (
    <header className="flex h-[64px] shrink-0 items-center gap-0.5 border-b border-line bg-panel px-2">
      ...
      <Sep />
      <Btn icon="＋" label="リング" onClick={() => actionAddRing(store)} title="リングを追加 (N)" />
      <Sep />
      <Btn icon="◌" label="デザイン" onClick={() => store.setView({ mode: "design" })} active={view.mode === "design"} title="リングごとの形状を表示 (S で切替)" />
      <Btn icon="◉" label="ステンシル" onClick={() => store.setView({ mode: "stencil" })} active={view.mode === "stencil"} title="結合・ブリッジ後の切り抜き形状を表示 (S で切替)" />
      <Btn icon="▦" label="グリッド" onClick={() => store.setView({ grid: !view.grid })} active={view.grid} title="グリッド (G)" />
      <Btn icon="✳" label="ガイド" onClick={() => store.setView({ guides: !view.guides })} active={view.guides} title="中心・放射ガイド" />
      ...
      <div className="mr-2 max-w-[220px] truncate text-[12px] text-ink-2" title={name}>
        {name}
      </div>
      <Btn icon="⇪" label="共有URL" onClick={() => openDialog("share")} />
      <Btn icon="?" label="ヘルプ" onClick={() => openDialog("help")} />
      <Btn icon="⬢" label="SVG出力" onClick={() => actionExportSVG(store, render)} primary title="レーザー加工用SVGを書き出す (⌘⇧S)" />
    </header>
  );
}

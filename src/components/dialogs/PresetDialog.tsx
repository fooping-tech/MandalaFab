...
function Thumb({ id }: { id: string }) {
  const data = useMemo(() => {
    ...
    return { path: r.finalPath, bridges: r.bridgePath, w: p.sheet.width, h: p.sheet.height };
  }, [id]);
  return (
    <svg viewBox={`${-data.w / 2} ${-data.h / 2} ${data.w} ${data.h}`} className="h-36 w-full rounded bg-white">
...
export function PresetDialog({ store, open, onClose }: { store: EditorStore; open: boolean; onClose: () => void }) {
  return (
    <Dialog open={open} onClose={onClose} title="プリセット" width={720}>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {open &&
          PRESETS.map((p) => (
            ...
            >
              <Thumb id={p.id} />
              <div className="mt-1.5 text-[12px] font-medium">{p.label}</div>

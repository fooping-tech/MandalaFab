...
- **スマホ対応**: 幅 860 px 以下では 1 カラム表示。ツールバーはアイコンのみで横スクロール、ツリーと Inspector は下部バーからシートとして開閉、ステータスバーの代わりに「ツリー / 編集 / 表示切替 / プレビュー / 全体」の下部バー。キャンバスは 1 本指でパン、2 本指でピンチズームと移動、タップで選択、長押しで操作メニュー、「範囲選択」ボタンで 1 本指ドラッグを矩形選択に切替。ハンドルはタッチ用に大きく、ダイアログは画面幅に収まる
- **セクタ軸へのスナップ**: 位置ハンドルをセクタの中心線（y = 0）の近くへ動かすと吸着し、ミラー付きリングでも要素が 2 つに分かれず 1 つのまま中心に置ける（Alt で無効、タッチは吸着範囲が広め）
- **使い方（？）**: エディタ右上の「？」で使い方を表示。ファイルを開く・保存する・SVG を書き出す手順を先頭にまとめている
- **ロックと非表示**: 左のツリーの各行（🔒 / ◉）、右クリックメニュー、Inspector で要素・リングをロック / 非表示にできる。ロック中はキャンバスで選択・移動・ハンドル操作・矢印キーが効かず、範囲選択と ⌘A からも外れる（ツリーからは選択・解除できる）。リングのロックは要素にも及ぶ。複数選択でもまとめて切替
- **Output Polarity（Stencil / Positive）**: ツールバーまたは Inspector の「出力」で切替。**Stencil** は曼荼羅をシートから抜く（背景シートが残る、島はブリッジで接続）。**Positive** は曼荼羅そのものを 1 つの部品として切り残す（外側を除去）。Positive では離れた図柄を曲線の帯（コネクタ）で自動的に繋ぎ（デザインの回転対称・鏡映対称を保つように、コネクタは対称群の像ごとまとめて追加）、内部では design / material / cut のジオメトリを分けて扱う（material = Stencil: シート − 図柄、Positive: 図柄 ∪ コネクタ。cut は material の境界）。Positive 専用の検証（分離部品・孤立した飾り・穴の中の浮いた材料・くびれ・細い先端・細すぎる材料・狭い隙間、最小接続幅は設定可能）。SVG 書き出しは極性に応じたカットライン。参照画像インポートの「C: Positive Cutout」で黒い模様を残す材料として取り込める
- **手描き（ペン / 指 / マウス）**: キャンバス右下の「✎ 描く」を ON にしてなぞると、線が Bézier 要素（開いた帯、始点に戻ると閉じた形）として選択中のリングに追加される。ペンの揺れは等間隔リサンプリングとガウス平滑化で均してから 3 次曲線にあてはめるので、少ない制御点のなめらかな曲線になる。描いた位置のセクタに入り、リングがなければ描いた半径に新しいリングを作る
- **数値入力**: Inspector の数値はスライダーバーのドラッグ、− / + ボタン（長押しで連続、Shift で 10 倍）、左端のグリップを左右にドラッグ、↑↓ キー、直接入力のどれでも変えられる（タッチ向けに大きめ）
- **ツールバーの「⋯ その他」**: 幅が足りないときは優先度の低いボタンがメニューに畳まれ、「SVG出力」は常に見える
